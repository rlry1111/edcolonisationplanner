...
class MainWindow(ttk.Window):
    ...
    def create_optimization_criterion_choice(self):
        ...
        frame2.pack()
        advancedframe = ttk.LabelFrame(frame2, text="Advanced objective", padding=2)
        ...
        ToolTip(canvas, text="Set your own custom objective function.\nThe objective function is what the program tries to maximize/minimize (depending on what you select).\nSupported operators:\n+, -, /, *, ^, ln() (natural logarithm)\n\nIf brackets are not present, standard order of operations will be followed\nBrackets after the ln are required, for the program to recognize what to take the logarithm of\nSpaces can be placed anywhere\n\nUse letters to represent the system scores:\ni: Initial population increase\nm: Maximum population increase\ns: Security\nt: Tech level\nw: Wealth\na: Standard of living\nd: Development level\nc: Construction cost\n\nExample inputs:\nw*t^2 maximizes/minimizes wealth * (tech level squared)\n(d + ln(s^ w))/c maximizes/minimizes (development level + ln(standard of living ^ wealth)) / construction cost")
        checkbox = ttk.Checkbutton(frame, variable=self.advancedobjective, command=lambda: (dropdown.config(state='disabled'), directionswitch.config(state='normal'), entry.config(state='normal')) if self.advancedobjective.get() else (dropdown.config(state='normal'), directionswitch.config(state='disabled'), entry.config(state='disabled')))
