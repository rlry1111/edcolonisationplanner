...
class ExportWindow(tkinter.Toplevel):
    ...
    def update_text(self, *args):
        ...
        if not not_mixed:
            self.order = ordering.get_mixed_ordering_from_result(self.extracted)
        else:
            include_initial_state = self.include_initial_state_var.get()
            self.order = ordering.get_ordering_from_result(self.extracted,
                                                           with_already_present=include_initial_state)
        self.daftmav_str = daftmav.export_ordering(self.order)
